...
class QuestionClassifier:
    # 定义初始函数
    def __init__(self):
        cur_dir = "D:\\PycharmProjects\\chatbot_KnowledgeGrapg_modify"
        # 导入的词典
        #　特征词路径
        ...
        # 这个wdtype_dict得到的是词的分类的结果，以字典存储
        # 问句疑问词
        ...
        self.drug_qwds = ['药', '药品', '用药', '胶囊', '口服液', '炎片','吃']
    ...
    def classify(self, question):
        ...
        for type_ in medical_dict.values():
            types += type_
        # types列表里面的数据举个例子是这样的['l', 'i', 's', 'i', '2', '0', '男'],这个地方有不明白
        # 返回结果列表
        ...
        if self.check_words(self.symptom_qwds, question) and ('disease' in types):
            # 这里的check_words方法就是为了遍历symptom_qwds，然后判断特征词是否在问句当中
